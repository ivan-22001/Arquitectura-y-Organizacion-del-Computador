...
{
    char* mon = moneda?"USD":"ARG";
    fprintf(pfile, "%s, %u, %s, %u\n", productos[id], precio, mon, stock);
}
...
void procesar_e_imprimir_catalogo(FILE *pfile)
{
	...
	int i = 0;
	while(i < 32){
		uint32_t val 	 = catalogo[i];
		uint32_t val2 	 = catalogo[i+1];
		uint32_t id1 	 = val >> 24;
		uint32_t id2 	 = (val >> 16) & 0x000000FF;
		uint32_t precio1 = (val & 0x0000FFFF) >> 4; 
		uint32_t precio2 = ((val & 0x00000007) << 9) | (val2 >> 23);
		uint32_t moneda1 = (val  & 0x00000008) >> 3;
		uint32_t moneda2 = (val2 & 0x00400000) >> 22;
		uint32_t stock1  = (val2 & 0x003FF800) >> 11;
		uint32_t stock2  = (val2 & 0x000007FF);
